...
# Load data
df = pd.ExcelFile(PATH.joinpath("comptab_2018-01-29 16_00_comma_separated.csv"))

importer_info = {}
importer_options_list = []
for i in range(len(available_indicators)):
    importer_info['label'] = str(available_indicators[i])
    importer_info['value'] = str(df.sheet_names[i])
...
def description_card():
    ...
    return html.Div(
        id="description-card",
        children=[
            html.H1("Welcome to CIETS Trade Analytics Dashboard",
